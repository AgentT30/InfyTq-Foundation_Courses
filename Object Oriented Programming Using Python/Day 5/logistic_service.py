...
class Logistics(metaclass=ABCMeta):
    ...
    def generate_consumer_id(self):
        cons_id = Logistics.__counter
        cons_id += 1
        self.__consumer_id = cons_id
        Logistics.__counter = cons_id
...
class PassengerLogistics(Logistics):
    ...
    def validate_vehicle_type(self):
        for index in range(0, len(PassengerLogistics.__list_vehicle)):
            if(PassengerLogistics.__list_vehicle[index] == self.__vehicle_type.upper()):
                return index
        return -1

    def calculate_bill_amount(self):
        if self.validate_meter_reading() and not self.validate_vehicle_type() == -1:
            self.generate_consumer_id()
            distance_travelled = self.get_end_reading() - self.get_start_reading()
            index_of_vehicle = PassengerLogistics.__list_vehicle.index(
                self.__vehicle_type)
            bill_amount = PassengerLogistics.__list_minimum_charge[index_of_vehicle]
            if distance_travelled <= 100:
                bill_amount += PassengerLogistics.__list_charge_for_hundred[index_of_vehicle] * \
                    distance_travelled
            elif distance_travelled > 100:
                bill_amount += PassengerLogistics.__list_charge_after_hundred[index_of_vehicle] * \
