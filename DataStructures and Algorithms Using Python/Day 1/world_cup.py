...
class Game:
    ...
    def sort_players_based_on_experience(self):
        self.__players_list.sort(
            key=lambda x: x.get_experience(), reverse=True)

    def shift_player_to_new_position(self, old_index_position, new_index_position):
        old_player = self.__players_list.pop(old_index_position)
        self.__players_list.insert(new_index_position, old_player)
