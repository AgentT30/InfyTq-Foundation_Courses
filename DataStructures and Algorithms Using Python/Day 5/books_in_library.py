...
class Library:
    ...
    def sort_item_list_by_author(self, new_item_list):
        # print(new_item_list[1].get_author_name())
        temp = []
        for i in range(len(new_item_list)):
            temp.append([new_item_list[i].get_item_name(),
                         new_item_list[i].get_author_name(),
                         new_item_list[i].get_published_year()])
        temp.sort()
        ...
        new_books_list = []
        for i in temp:
            new_books_list.append(Item(i[0], i[1], i[2]))
        return new_books_list

    def add_new_items(self, new_item_list):
        new_books = self.sort_item_list_by_author(new_item_list)
        Library(new_books)

    def sort_items_by_published_year(self):
        for i in range(0, len(self.__item_list)):
            for j in range(0, len(self.__item_list)-i-1):
                if (self.__item_list[j][2] > self.__item_list[j + 1][2]):
                    tempo = self.__item_list[j]
                    self.__item_list[j] = self.__item_list[j + 1]
                    self.__item_list[j + 1] = tempo
